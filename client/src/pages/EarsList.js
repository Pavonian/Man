...
class EarsList extends Component {
  // Init component
  constructor(props) {
    super(props);
    this.state = {
  ...
  confirmDialogDelete(id) {
    this.props.actionsEars.deleteEars(this.state.idDelete).then(data => {
      this.props.actionsEars.loadEarsList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/earses/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.EarsListReducer.listEars
  };
}
